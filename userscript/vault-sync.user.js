// ==UserScript==
// @name         Vault 一键备份助手 (原生 GitHub 风格 + 右键管理面板版)
// @namespace    https://github.com/owwk-backup
// @version      2.0.0
// @description  左键一键备份，右键开启高级面板：查看清单、删除项目、触发同步与 Worker 配置，彻底告别 alert
// @author       owwk-backup
// @match        *://github.com/*
// @match        *://*.github.com/*
// @include      *://github.com/*
...
(function () {
    ...
    .vault-modal-body {
        padding: 20px; overflow-y: auto; flex: 1; min-height: 260px;
    }
    .vault-item-card {
    ...
    function openDashboardModal() {
        ...
        async function renderReposTab() {
            ...
            body.innerHTML = `<div style="text-align:center; padding: 40px; color: var(--fgColor-muted, #8b949e);">⏳ 正在从私有配置仓读取清单...</div>`;
            ...
            try {
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
                    <div style="font-size: 13px; color: var(--fgColor-muted, #8b949e);">
                        当前组织已归档 <strong>${list.length}</strong> 个项目
                    </div>
                ...
                if (list.length === 0) {
                    html += `<div style="text-align:center; padding: 50px 0; color: #8b949e;">暂无任何备份项目，可点击页面顶栏的 [备份] 按钮快速添加！</div>`;
                } else {
                    list.forEach(item => {
                        ...
                        html += `
                        <div class="vault-item-card" data-repo="${targetName}">
                            <div style="min-width: 0; flex: 1;">
                                <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
                                    <span class="vault-tag ${tagClass}">${item.type}</span>
                                    <strong style="font-size: 14px;">${targetName}</strong>
                                    ${item.private ? '<span style="font-size: 11px; opacity: 0.6;">🔒 Private</span>' : ''}
                                </div>
                            ...
                            <div>
                                <button type="button" class="vault-btn vault-btn-danger vault-delete-item-btn" data-target="${targetName}">
                                    🗑️ 移除
                                </button>
                            </div>
                        </div>
        ...
        function renderSettingsTab() {
            ...
            <div style="max-width: 520px; margin: 0 auto; padding: 10px 0;">
                ...
                <input type="text" id="vault-cfg-url" class="vault-input" 
                       ...
                       value="${config.workerUrl.includes('your-worker-subdomain') ? '' : config.workerUrl}" />
